/*  Prototype JavaScript framework, version 1.7.0.6
...
(function() {
  ...
  function intersect(array) {
    return this.uniq().filter(function(item) { // FIX
...
Autocompleter.Base = Class.create({
  baseInitialize: function(element, update, options) {
    ...
    var prebind = $w('onBlur onKeyPress onKeyDown onMouseDown onMouseLeave fixIEOverlapping onObserverEvent hide onHover onClick');
...
Autocompleter.Local = Class.create(Autocompleter.Base, {
  ...
  selector: function(instance) {
    ...
    var o = instance.options, entry = o.ignoreCase ? instance.getToken().toLowerCase() : instance.getToken(), entrylen = entry.length;

    for (var i = 0; i < o.array.length && ret.length < o.choices; i++) {

      var elem = o.array[i], elem2 = o.ignoreCase ? elem.toLowerCase() : elem;
      var foundPos = elem2.indexOf(entry);
...
//(function(GLOBAL) {GLOBAL.Control = GLOBAL.Control || {};})(this);
...
Control.Slider = Class.create({
  initialize: function(handle, track, options) {
    ...
    this.handles.each(function(h, i) {
      ...
      this.setValue(parseFloat((Object.isArray(options.sliderValue) ? options.sliderValue[i] : options.sliderValue) || this.range.start), i);
...
if (false && Sound.html5audio) {
  // in Firefox and Opera does not support mp3
  Sound.template = new Template('<audio id="sound_#{track}_#{id}" src="#{url}" preload="auto" autobuffer="autobuffer" autoplay="autoplay"/>');
  ...
  (function() {
    ...
    if (plugin('QuickTime')) {
      ...
      Sound.template = new Template('<embed id="sound_#{track}_#{id}" type="audio/x-pn-realaudio-plugin" style="height:0" src="#{url}" loop="false" autostart="true" hidden="true"/>');
    } else {
      Sound.play = function() {
      };
    }
  ...
  Sound.template = new Template('<embed id="sound_#{track}_#{id}" style="height:0" src="#{url}" loop="false" autostart="true" hidden="true"/>');
}
...
(function() {
  ...
  var module_reserved_names = 'selfExtended|self_extended|selfIncluded|self_included|extend|include';
  //module_reserved_names_arr = module_reserved_names.split('|')
  //module_reserved_names_str = '|' + module_reserved_names + '|'
   ...
   * Invoke callback `module.selfExtended(this)` or `module.self_extended(this)` after extension.
   * @param {Object} module
   */
  function extendWithModule(module) {
    ...
    (module.selfExtended || module.self_extended || Prototype.emptyFunction).call(module, this);
  }
  ...
  function wrap(method, originalMethod) {
    method = method.wrap(originalMethod);
    method.valueOf = originalMethod.valueOf.bind(originalMethod);
   ...
   * Invoke callback `module.selfIncluded(this)` or `module.self_included(this)` after inclusion.
   * @param {Object} module
   */
  function includeModule(module) {
    ...
    for (var i = 0, length = names.length; i < length; i++) {
      ...
      if (Object.isFunction(method) && method.argumentNames()[0] == "$super") {
        if (name in this.prototype) {
          ...
          var ancestor = findClassWithMethod(name, this.ancestors);
          if (ancestor) {
    ...
    (module.selfIncluded || module.self_included || Prototype.emptyFunction).call(module, this);
  }

  /*
   function addMethods(source) {
   var ancestor = this.superclass && this.superclass.prototype, properties = clearModuleNames(source);
   for (var i = 0, length = properties.length; i < length; i++) {
   var property = properties[i], value = source[property];
  ...
  Class.create = function() {
    ...
    function klass() {
      var prebind = this.prebind, method;
 ...
 * @param {Number} timeout - execution interval in seconds
 */
Function.prototype.periodical = function(timeout) {
  var args = $A(arguments);
  timeout = 1000 * args.shift();
  var timer = new Number(setInterval(this.bind.apply(this, [this].concat(args)), timeout));
  timer.stop = function() {
    clearInterval(this);
  };
  return timer;
...
Function.prototype.chain = function(func) {
  var args = $A(arguments), current = this;
  func = args.shift();
  return function() {
...
// for original ajax.js
Ajax.Request.addMethods({
  /**
   * Allows additional Ajax.Responders `onSuccess` and `onFailure`.
   *
   * Full list of guaranteed responders:
   * `onCreate`, `onException`, `onSuccess`, `onFailure`, `onComplete`.
   *
...
/* // for fixed ajax.js
 ...
 respondToReadyState: function(readyState) {
 var state = Ajax.Request.Events[readyState], callbacks = [], response = new Ajax.Response(this);
 if (this._aborted) {state = 'Complete'; callbacks.push(this._timedout ? 'Timeout' : 'Abort');}
 if (state == 'Complete') {
...
Ajax.Updater.addMethods({
  initialize: function($super, container, url, options) {
    ...
    options.evalScripts = (options.evalScripts !== false);
    options.showLoading = (options.showLoading !== false);
    options.clearTarget = (options.clearTarget !== false);

    var target = $(this.container.success), onComplete = options.onComplete;
    if (target) {
    ...
    options.onComplete = (function(response, json) {
      this.updateContent(response.responseText);
      var target = $(this.container.success);
      if (target && options.showLoading) {
...
(function() {
   ...
   */
  var getIntStyles = function(element) { // stub
    return 0;
  };
  ...
  /**
   * Return margin size of element at specified side.
   *
   * @param {Element/String} element
   * @param {String} s - side ('top', 'right', 'bottom', 'left')
   * @return {Number} margin
  function getMargin(element, s) {
    return getIntStyle(element, "margin-" + s);
  }
   */

  /**
   * Return border size of element at specified side.
   *
   * @param {Element/String} element
   * @param {String} s - side ('top', 'right', 'bottom', 'left')
   * @return {Number} border
  function getBorder(element, s) {
    return getIntStyle(element, "border-" + s + "-width");
  }
   */

  /**
   * Return padding size of element at specified side.
   *
   * @param {Element/String} element
   * @param {String} s - side ('top', 'right', 'bottom', 'left')
   * @return {Number} padding
  function getPadding(element, s) {
    return getIntStyle(element, "padding-" + s);
  }
   */

  /**
   * Return full padding (margin + border + padding)
   * of element at specified side.
   *
   * @param {Element/String} element
   * @param {String} s - side (top, right, bottom, left)
   * @return {Number} full padding
  function getFullPadding(element, s) {
    //element = $(element);
    //return getPadding(element, s) + getBorder(element, s) + getMargin(element, s);
    return getIntStyles(element, "padding-" + s, "border-" + s + "-width", "margin-" + s);
  }
   */
...
(function() {
  ...
  var nextElement = 'nextElementSibling', previousElement = 'previousElementSibling', parentElement = 'parentElement';
   ...
   * @param {Number} index - index (starting from 0, default 0)
   */
  function traverse(element, method, selector, index) {
    element = $(element);
    if (Object.isNumber(selector)) {
      index = selector;
      selector = false;
    }
    ...
    if (selector) {
      while (node) {
        if (node.nodeType === 1 && Prototype.Selector.match(node, selector) && (index-- === 0)) {
          return Element.extend(node);
        }
        node = node[method];
      }
  ...
  function next(element, selector, index) {
    return traverse(element, nextElement, selector, index);
  }

  function previous(element, selector, index) {
    return traverse(element, previousElement, selector, index);
  }

  function up(element, selector, index) {
    return (arguments.length == 1) ? Element.extend($(element).parentNode) : traverse(element, parentElement, selector, index);
  }
 ...
 *   fitToParent, lastDescendant, isVisible, insertInto,
 *   getFocusableElements, getScrollParent.
 */
(function() {
  ...
  function isVisible(element) {
    ...
    var parent = element.parentNode;
    return parent &&
    internalGetStyle(element, 'display') !== 'none' &&
    internalGetStyle(element, 'visibility') !== 'hidden' &&
    //Element.getStyle(element, 'display') !== 'none' &&
    //Element.getStyle(element, 'visibility') !== 'hidden' &&
    isVisible(parent);
   ...
   * div|dl|fieldset|form|h[1-6]|ol|p|table|ul
  ...
  function getSimulatedOuterHTML(element) {
    ...
    if (/*element.childNodes.length === 0 && */(/br|hr|img|input|link|meta|param/).test(tag)) {
      outerHTML += '>';
    } else {
  ...
  function getFocusableElements(root) {
    ...
    while (element = elements[i++]) {
      ...
      if (tagName && (/^(?:a|input|select|textarea|button)$/i).test(tagName) && "hidden" != element.type && !element.disabled) {
  ...
  function getScrollParent(element) {
    for (var parent = element.parentNode; parent; parent = parent.parentNode/*offsetParent*/) {
...
(function() {
  ...
  function copyAttributes(src, dst) {
    var i, len = dst.attributes.length, attrCount = len, attrs = {}, attr;
    if (len) {
      for (i = 0; i < len; ++i) {
        // clear dst attribute when src attribute is undefined
        attr = dst.attributes[i];
        attrs[attr.nodeName] = null;
      }
    }
    len = src.attributes.length;
    attrCount += len;
    if (len) {
      for (i = 0; i < len; ++i) {
        attr = src.attributes[i];
        if (attr.nodeValue) {
  ...
  function updateTextNode(sNode, dNode) {
    if (dNode.nodeValue !== sNode.nodeValue) { // replace value
  ...
  function updateNodes(src, dst, options) {
    ...
    while (node.d) { // trim extra destination nodes
      node = removeDstNode(src, node.s, dst, node.d);
    }

    while (node.s) { // append extra source nodes
      next = node.s.nextSibling;
      src.removeChild(node.s);
  ...
  function updateSmoothly(element, content, options) {
    element = $(element).cleanWhitespace();
    if (content && content.toElement) {
      content = content.toElement(); // Element
    } else if (!Object.isElement(content)) {
      content = Object.toHTML(content); // String
      content.evalScripts.bind(content).defer();
      content = content.stripScripts();
    }
    content = new Element("div").update(content).cleanWhitespace();
...
Object.extend(Form.Methods, {
  /**
   * Set action of form to specified URL and call submit().
   *
   * Examples:
   *   $("idMyForm").submitForm("myAction.do");
   *   Form.submitForm("idMyForm");
   *   select onchange="Form.submitForm(this);"
   *   button onclick="return Form.submitForm(this);"
   *
   * @param {Form} form
   * @param {String} action
   */
  submitForm: function(form, action) {
    ...
    //if(!form){alert("Cannot find element '"+form+"'!"); return null;}
    if (Object.isString(action)) {
      form.action = action;
    }
    form.submit();
    return false; // prevent automatic submission
  },
  ...
  deserialize: function(form, source) {
    ...
    form.getElements().each(function(element) {
      //for (var name in source) {if (name == element.name) {element.setValue(source[name]);}}
      var name = element.name; // element.name element.readAttribute("name") element.getAttribute("name")
      if (name && !Object.isUndefined(source[name])) {
  ...
  /**
   * Form#getSuccessfulElements(@form) -> Array
   * @param {Form} form
   */
  getSuccessfulElements: function(form) {
...
Object.extend(Form.Element.Methods, {
   ...
   *
   * @param {Field} element - field to clear
   */
  clear: function(element) {
    element = $(element);
 ...
 * Field#getValue will always return \n.
 */
(function() {
...
Object.extend(Event, {
  ...
  /**
   * Where Event.isLeftClick is not working.
   *
   * @param {Event} event - click event
   */
  isNotLeftClick: function(/*Event*/event) {
    return event.stopped || Event.isMiddleClick(event) || Event.isRightClick(event);
  },
 ...
 */
(function() {
  // TODO keypress bugs in Opera, Chrome
  var eventMatchers = {
  ...
  function createEventDOM(options, element, eventName, eventType) {
    var event;
    if (eventType == 'HTMLEvents') {
      ...
      try {
        event = document.createEvent(eventType);
        event.initKeyEvent(eventName, options.bubbles, options.cancelable, window, options.ctrlKey, options.altKey, options.shiftKey, options.metaKey, options.keyCode, options.charCode);
      } catch (e) {
        try {
  ...
  function createEventIE(options, element) {
    var event = Object.extend(document.createEventObject(), Object.extend(options, {
      detail: 0,
  ...
  function createEventOther() {
    throw new Error('This browser does not support neither document.createEvent() nor document.createEventObject()');
  }

  var createEvent = document.createEvent ? createEventDOM : (document.createEventObject ? createEventIE : createEventOther);
  ...
  function simulate(element, eventName, options) {
    ...
    var event = createEvent(options, element, eventName, eventType);
...
/**
 * Make change and submit events bubble in all browsers. Works only for Event.Handler and element.on().
 */
(function() {
  ...
  if (!submitBubbles || !changeBubbles) {
    ...
    Event.Handler.prototype.initialize = Event.Handler.prototype.initialize.wrap(function(init, element, eventName, selector, callback) {
      init(element, eventName, selector, callback);
      // is the handler being attached to an element that doesn't support this event?
      if ((!submitBubbles && this.eventName == 'submit' && !isForm(this.element)) ||
      (!changeBubbles && this.eventName == 'change' && !isInput(this.element))) {
        // "submit" => "emulated:submit", "change" => "emulated:change"
  ...
  document.on("click", "form input[type=submit], form button[type=submit], form button:not([type])", function(event, button) {
    event.findElement("form").store("form:submit-button", button.name || false);
  });
})();
/**
 * Custom events:
